...
export default function BusinessHoursPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingId
        ? `/api/tickets/business-hours/${editingId}`
        : '/api/tickets/business-hours';
  ...
  const handleEdit = (bh: BusinessHours) => {
    ...
    setForm({
      ...
      schedule: bh.schedule.length ? bh.schedule : DEFAULT_SCHEDULE,
  ...
  const addHoliday = () => {
    ...
    setForm({
      ...form,
      holidays: [...form.holidays, { date: newHolidayDate, name: newHolidayName.trim() }],
    });
    setNewHolidayDate('');
    setNewHolidayName('');
  };
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Define when your team is available for SLA tracking
            </p>
          </div>
        </div>
        <button
      ...
      {showForm && (
        ...
        >
          <h2 className="text-lg font-semibold">
            {editingId ? 'Edit Schedule' : 'New Business Hours'}
          </h2>
                ...
                <div key={day.dayOfWeek} className="flex items-center gap-3">
                  <label className="flex w-28 items-center gap-2">
                    <input
                      type="checkbox"
                      checked={day.isOpen}
                      onChange={(e) => updateScheduleDay(day.dayOfWeek, 'isOpen', e.target.checked)}
                      className="rounded border-gray-300"
                    />
                    <span
                      className={`text-sm ${day.isOpen ? 'font-medium text-gray-900' : 'text-gray-400'}`}
                    >
                      {DAYS[day.dayOfWeek]}
                    </span>
                  </label>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center py-12">
          ...
          <p className="mt-2 text-sm text-gray-500">No business hours configured</p>
        </div>
      ) : (
            ...
            <div key={bh.id} className="rounded-lg border border-gray-200 bg-white p-5">
              ...
              <div className="mt-3 space-y-1">
                {(bh.schedule || []).map((day) => (
                  <div key={day.dayOfWeek} className="flex items-center gap-2 text-xs">
                    <span
                      className={`w-16 ${day.isOpen ? 'font-medium text-gray-700' : 'text-gray-400'}`}
                    >
              ...
              {bh.holidays?.length > 0 && (
                <div className="mt-2 text-xs text-gray-500">
                  {bh.holidays.length} holidays configured
                </div>
              )}
            </div>
          ))}
