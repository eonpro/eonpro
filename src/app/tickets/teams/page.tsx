...
interface Team {
  ...
  defaultSlaPolicy?: { id: number; name: string } | null;
  members: TeamMember[];
...
export default function TeamsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiFetch(url, {
        ...
        body: JSON.stringify({
          ...form,
          maxTicketsPerMember: form.maxTicketsPerMember
            ? parseInt(form.maxTicketsPerMember, 10)
            : null,
        }),
      });
      if (!res.ok) {
  ...
  const handleAddMember = async (teamId: number, userId: number) => {
    await apiFetch(`/api/tickets/teams/${teamId}/members`, {
      method: 'POST',
  ...
  const handleToggleLead = async (teamId: number, userId: number, isLead: boolean) => {
    await apiFetch(`/api/tickets/teams/${teamId}/members`, {
      method: 'POST',
      body: JSON.stringify({ userId, isLead: !isLead }),
    });
    await fetchData();
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Organize agents into teams for ticket routing</p>
          </div>
        </div>
        <button
          onClick={() => {
        ...
        >
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div>
              ...
              <input
                type="number"
                value={form.maxTicketsPerMember}
                onChange={(e) => setForm({ ...form, maxTicketsPerMember: e.target.value })}
            ...
            <div className="flex flex-col gap-2 pt-6">
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  ...
                  onChange={(e) => setForm({ ...form, roundRobinEnabled: e.target.checked })}
                  className="rounded border-gray-300"
                />
                Round-robin
              </label>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center py-12">
          ...
          <p className="mt-2 text-sm text-gray-500">No teams yet</p>
        </div>
      ) : (
                      ...
                      <button
                        onClick={() => {
                          ...
                          setForm({
                            ...
                            maxTicketsPerMember: team.maxTicketsPerMember
                              ? String(team.maxTicketsPerMember)
                              : '',
                          });
                          setShowForm(true);
                          setError(null);
                  ...
                  <div className="mt-3 flex -space-x-2">
                    {team.members.slice(0, 6).map((m) => (
                      <div
                        ...
                        title={`${m.user.firstName} ${m.user.lastName}${m.isLead ? ' (Lead)' : ''}`}
                      >
                        {m.user.firstName[0]}
                        {m.user.lastName[0]}
                      </div>
                      ...
                      <div key={m.id} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            {m.isLead && (
                              <Crown className="ml-1 inline h-3.5 w-3.5 text-yellow-500" />
                            )}
                          </div>
                        </div>
                        <div className="flex gap-1">
                          <button
                            onClick={() => handleToggleLead(team.id, m.userId, m.isLead)}
                            className={`rounded px-2 py-0.5 text-xs ${m.isLead ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600 hover:bg-yellow-50'}`}
                          >
                      ...
                      <div className="max-h-32 space-y-1 overflow-y-auto">
                        {users
                          ...
                          .map((u) => (
                            <button
                              key={u.userId}
                              onClick={() => handleAddMember(team.id, u.userId)}
