...
export default function SLAPoliciesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (bhRes.ok) {
        const d = await bhRes.json();
        setBusinessHoursList(d.businessHours || []);
      }
    } catch {
      /* */
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingId
        ? `/api/tickets/sla-policies/${editingId}`
  ...
  const handleEdit = (p: SlaPolicy) => {
    ...
    setForm({
      ...
      respectBusinessHours: p.respectBusinessHours,
      escalateOnBreach: p.escalateOnBreach,
      warningThresholdPct: String(p.warningThresholdPct),
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Response and resolution time targets</p>
          </div>
        </div>
        <button
          onClick={() => {
          ...
          <div className="grid gap-4 md:grid-cols-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, firstResponseMinutes: e.target.value })}
                required
                min="1"
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
              />
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div>
              ...
              <select
                ...
                onChange={(e) => setForm({ ...form, businessHoursId: e.target.value })}
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
              >
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">Warning at (%)</label>
              <input
                type="number"
            ...
            <div className="flex flex-col gap-2 pt-6">
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={form.escalateOnBreach}
                  onChange={(e) => setForm({ ...form, escalateOnBreach: e.target.checked })}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center py-12">
          ...
          <p className="mt-2 text-sm text-gray-500">No SLA policies yet</p>
        </div>
      ) : (
              ...
              {policies.map((p) => (
                <tr key={p.id}>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600">
                    {p.priority ? p.priority.replace(/_/g, ' ') : 'All priorities'}
                    {p.category ? ` / ${p.category.replace(/_/g, ' ')}` : ''}
                  </td>
                  <td className="px-4 py-3 text-right text-sm font-medium text-gray-900">
                    {formatTime(p.firstResponseMinutes)}
                  </td>
                  <td className="px-4 py-3 text-right text-sm font-medium text-gray-900">
                    {formatTime(p.resolutionMinutes)}
                  </td>
