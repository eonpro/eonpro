    ...
    try {
      ...
      if (response.ok) {
        ...
        const outstanding = allInvoices.filter(
          (i: Invoice) => i.status === 'OPEN' || i.status === 'SENT'
        );
        const overdue = outstanding.filter(
          (i: Invoice) => i.dueDate && new Date(i.dueDate) < new Date()
        );
        ...
        const paidTotal = paidInvoices.reduce(
          (sum: number, i: Invoice) => sum + (i.amountPaid || i.amount || 0),
          0
        );
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-gray-500">Outstanding</p>
        </div>

        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="w-fit rounded-lg bg-red-50 p-2">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-gray-500">Paid This Month</p>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-gray-500">Avg Payment Time</p>
        </div>
      </div>
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredInvoices.length === 0 ? (
                    ...
                    filteredInvoices.map((invoice) => {
                      ...
                      const invoiceNumber =
                        invoice.invoiceNumber || invoice.stripeInvoiceId || `INV-${invoice.id}`;
                      const isIncompletePatient =
                        invoice.patient?.firstName === 'Unknown' ||
                        ...
                        <tr key={invoice.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex flex-col gap-1">
                              <span
                                className={`inline-flex w-fit rounded-full px-2 py-1 text-xs font-medium ${getStatusColor(invoice.status)}`}
                              >
                                {invoice.status}
                              </span>
                              {hasRefund && (
                                <span className="inline-flex w-fit rounded-full bg-red-100 px-2 py-1 text-xs font-medium text-red-700">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-1">
                              ...
                              {invoice.status === 'DRAFT' && (
                                <button className="rounded p-1.5 hover:bg-gray-100" title="Send">
                                  <Send className="h-4 w-4 text-gray-500" />
                                </button>
                              )}
                              <button className="rounded p-1.5 hover:bg-gray-100" title="Download">
                                <Download className="h-4 w-4 text-gray-500" />
                              </button>
                              <button className="rounded p-1.5 hover:bg-gray-100" title="More">
              ...
              <div className="flex items-center justify-between border-t border-gray-200 px-6 py-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={page === 1}
                    className="rounded border border-gray-200 p-2 hover:bg-gray-50 disabled:opacity-50"
                  >
