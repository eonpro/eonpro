...
export default function ShipmentMonitorPage() {
  ...
  const handleBulkRefresh = async () => {
    ...
    setBulkResult(null);
    try {
      const res = await apiFetch('/api/super-admin/shipment-monitor/refresh-all', {
        method: 'POST',
      });
      if (res.ok) {
        const data = await res.json();
        setBulkResult({
          ...
          totalPolled: data.totalPolled,
          totalNoData: data.totalNoData || 0,
        });
        await fetchData(tab, pagination.page, search, dateRange, selectedClinicId);
      }
    } catch {
  ...
  const handleViewProof = async (trackingNumber: string) => {
    ...
    try {
      const res = await apiFetch(
        `/api/super-admin/shipment-monitor/delivery-proof?trackingNumber=${encodeURIComponent(trackingNumber)}`
      );
      const data = await res.json();
      if (data.success && data.documentBase64) {
        setProofModal({
          ...
          format: data.documentFormat || 'PNG',
          loading: false,
          error: null,
        });
      } else {
        setProofModal({
          ...
          format: 'PNG',
          loading: false,
          error: data.message || 'Proof of delivery not available for this shipment',
        });
      }
    } catch {
      setProofModal((prev) =>
        prev ? { ...prev, loading: false, error: 'Failed to load delivery proof' } : null
      );
    }
  };
  const handleExport = () => {
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Track every shipment from label creation through delivery across all clinics
          </p>
        </div>
      ...
      {bulkResult && (
        <div
          className={`mb-4 flex items-center justify-between rounded-xl border px-4 py-3 ${bulkResult.totalNoData > 0 && bulkResult.totalUpdated === 0 ? 'border-amber-200 bg-amber-50' : 'border-emerald-200 bg-emerald-50'}`}
        >
          <div
            className={`flex items-center gap-2 text-sm ${bulkResult.totalNoData > 0 && bulkResult.totalUpdated === 0 ? 'text-amber-800' : 'text-emerald-800'}`}
          >
            ...
            <span>
              FedEx tracking: <strong>{bulkResult.totalPolled}</strong> sent,{' '}
              ...
              {bulkResult.totalNoData > 0 && (
                <>
                  {' '}
                  · <strong>{bulkResult.totalNoData}</strong> no data from FedEx
                </>
              )}
            </span>
          </div>
          <button onClick={() => setBulkResult(null)} className="text-xs hover:underline">
            Dismiss
          </button>
        </div>
      )}
          ...
          return (
            <button
              ...
              className={`flex flex-col rounded-xl border-l-4 bg-white p-4 text-left shadow-sm transition-all ${t.borderColor} ${t.bgHover} ${
                isActive
                  ? 'shadow-md ring-2 ring-[#4fa77e]'
                  : 'border border-b border-r border-t border-gray-100'
              }`}
            >
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs text-gray-600">
                        {s.clinicName || `Clinic #${s.clinicId}`}
                      </td>
                      ...
                      {(tab === 'in_transit' || tab === 'shipped') && (
                        <td className="px-4 py-3 text-xs text-gray-600">
                          ...
                          !isSameDay(s.estimatedDelivery, s.createdAt)
            ...
            <div className="p-6">
              {proofModal.loading ? (
                <div className="flex flex-col items-center py-16">
                  <div className="border-3 h-8 w-8 animate-spin rounded-full border-gray-200 border-t-[#4fa77e]" />
                ...
                <div className="flex flex-col items-center py-16">
                  ...
                  <p className="mt-2 text-xs text-gray-400">
                    FedEx may not provide delivery photos for all shipments. Signature Proof of
                    Delivery is typically available for Express and Ground shipments.
                  </p>
                </div>
                  ...
                  {proofModal.format === 'PDF' ? (
                    <div className="flex flex-col items-center gap-4">
                      <p className="text-sm text-gray-600">
                        Delivery proof is available as a PDF document.
                      </p>
                      <a
