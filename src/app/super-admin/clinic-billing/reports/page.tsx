...
import { FileText, DollarSign, Clock, TrendingUp, Download, Loader2 } from 'lucide-react';
...
export default function BillingReportsPage() {
  ...
  const [revenueData, setRevenueData] = useState<{
    ...
    topClinics: Record<string, unknown>[];
  } | null>(null);
  ...
  const fetchRevenueData = useCallback(async () => {
    ...
    try {
      const [trendRes, clinicsRes] = await Promise.all([
        ...
        apiFetch(
          `/api/super-admin/billing-analytics?type=top-clinics&limit=10&startDate=${startDate}&endDate=${endDate}`
        ),
      ]);
  ...
  const fetchCollectionData = useCallback(async () => {
    ...
    try {
      const res = await apiFetch(
        `/api/super-admin/billing-analytics?type=collection&startDate=${startDate}&endDate=${endDate}`
      );
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-end">
        <button
          onClick={exportCSV}
          className="flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2 text-gray-700 shadow-sm hover:bg-gray-50"
        >
      ...
      <div className="mb-6 flex flex-wrap items-center gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#4fa77e] focus:outline-none focus:ring-2 focus:ring-[#4fa77e]/20"
          />
        </div>
        <div className="flex items-center gap-2">
          ...
          {[
            {
              ...
              fn: () => {
                const n = new Date();
                setStartDate(instantToCalendarDate(new Date(n.getFullYear(), n.getMonth(), 1)));
                setEndDate(instantToCalendarDate(n));
              },
            },
            {
              label: 'Last 3M',
              fn: () => {
                ...
                setStartDate(instantToCalendarDate(s));
                setEndDate(instantToCalendarDate(n));
              },
            },
            {
              ...
              fn: () => {
                const n = new Date();
                setStartDate(instantToCalendarDate(new Date(n.getFullYear(), 0, 1)));
                setEndDate(instantToCalendarDate(n));
              },
            },
          ].map((p) => (
...
function FeeActivityTab({
  data,
}: {
  data: { summary: Record<string, number>; events: Record<string, unknown>[] };
}) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        {[
          {
            label: 'Prescription Fees',
            value: summary.totalPrescriptionFees,
            count: summary.prescriptionCount,
            color: 'text-[#4fa77e]',
          },
          {
            label: 'Transmission Fees',
            value: summary.totalTransmissionFees,
            count: summary.transmissionCount,
            color: 'text-blue-600',
          },
          {
            label: 'Admin Fees',
            value: summary.totalAdminFees,
          ...
          <thead className="bg-gray-50">
            <tr>
              {['Date', 'Clinic', 'Type', 'Provider', 'Status', 'Amount'].map((h) => (
                <th
                  key={h}
                  className={`px-4 py-3 text-xs font-medium uppercase text-gray-500 ${h === 'Amount' ? 'text-right' : 'text-left'}`}
                >
                  {h}
                </th>
              ))}
              ...
              .map((e: Record<string, unknown>, i: number) => (
                <tr key={i} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-900">
                    {(e.clinic as Record<string, string>)?.name}
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${e.feeType === 'PRESCRIPTION' ? 'bg-green-100 text-green-700' : e.feeType === 'TRANSMISSION' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'}`}
                    >
                      {e.feeType as string}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-600">
                    {(e.provider as Record<string, string>)?.firstName
                      ? `${(e.provider as Record<string, string>).firstName} ${(e.provider as Record<string, string>).lastName}`
                      : '-'}
                  </td>
                  <td className="px-4 py-3 text-xs text-gray-500">{e.status as string}</td>
                  <td className="px-4 py-3 text-right text-sm font-medium text-gray-900">
                    {formatCurrencyFull(e.amountCents as number)}
                  </td>
                </tr>
              ))}
          </tbody>
...
function RevenueTab({
  ...
  data: { trend: Record<string, number>[]; topClinics: Record<string, unknown>[] };
}) {
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={data.trend}>
            ...
            <Tooltip
              formatter={
                ((v: number | undefined, name: string) => [formatCurrency(v ?? 0), name]) as any
              }
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                {['Rank', 'Clinic', 'Invoiced', 'Paid', 'Outstanding', 'Collection Rate'].map(
                  (h) => (
                    <th
                      key={h}
              ...
              {(data.topClinics as Record<string, unknown>[]).map((c, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm font-medium text-gray-900">
                    {c.clinicName as string}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-900">
                    {formatCurrency(c.totalInvoicedCents as number)}
                  </td>
                  <td className="px-4 py-3 text-sm text-green-600">
                    {formatCurrency(c.totalPaidCents as number)}
                  </td>
                  <td className="px-4 py-3 text-sm text-yellow-600">
                    {formatCurrency(c.outstandingCents as number)}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="h-2 flex-1 overflow-hidden rounded-full bg-gray-200">
                        <div
                          className="h-full rounded-full bg-[#4fa77e]"
                          style={{ width: `${c.collectionRate as number}%` }}
                        />
                      </div>
                      <span className="text-xs font-medium text-gray-600">
                        {c.collectionRate as number}%
                      </span>
                    </div>
                  </td>
                </tr>
...
function ARAgingTab({ data }: { data: Record<string, unknown>[] }) {
  const totalOutstanding = (data as { amountCents: number }[]).reduce(
    (s, b) => s + b.amountCents,
    0
  );
  const totalInvoices = (data as { invoiceCount: number }[]).reduce(
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={data} layout="vertical">
              ...
              />
              <Tooltip
                formatter={((v: number | undefined) => [formatCurrency(v ?? 0), 'Amount']) as any}
                contentStyle={{ borderRadius: 12, border: '1px solid #e5e7eb' }}
              />
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie
                data={data.filter((d: Record<string, unknown>) => (d.amountCents as number) > 0)}
                cx="50%"
                cy="50%"
                innerRadius={50}
                outerRadius={90}
              ...
              <Tooltip
                formatter={((v: number | undefined) => formatCurrency(v ?? 0)) as any}
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            {(
              data as { label: string; range: string; invoiceCount: number; amountCents: number }[]
            ).map((b, i) => (
              <tr key={i} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-3 text-sm text-gray-600">
                  {totalOutstanding > 0
                    ? `${Math.round((b.amountCents / totalOutstanding) * 100)}%`
                    : '0%'}
                </td>
              </tr>
            ))}
        ...
        {[
          {
            ...
            color: data.collectionRate >= 80 ? 'text-green-600' : 'text-yellow-600',
          },
          ...
          {
            label: 'Outstanding',
            value: formatCurrency(data.totalOutstandingCents),
            color: 'text-yellow-600',
          },
          {
            label: 'Overdue',
            ...
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie
                  ...
                  stroke="none"
                >
                  {data.paymentMethodBreakdown.map((_e, i) => (
                    <Cell key={i} fill={COLORS[i % COLORS.length]} />
                  ))}
                </Pie>
                <Tooltip
              ...
              {data.paymentMethodBreakdown.map((pm) => {
                const pct =
                  data.totalPaidCents > 0
                    ? Math.round((pm.amountCents / data.totalPaidCents) * 100)
                    : 0;
                return (
                  <div key={pm.method}>
                    <div className="mb-1 flex justify-between text-sm">
                      ...
                      <span className="text-gray-500">
                        {pm.count} payments - {formatCurrency(pm.amountCents)}
                      </span>
                    </div>
