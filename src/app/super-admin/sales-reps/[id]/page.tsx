...
export default function SalesRepDetailPage() {
  ...
  const handleExportCsv = () => {
    if (!data) return;
    const r = data.rep;
    const dl = formatDateRange(data.dateRange.startDate, data.dateRange.endDate);
    ...
    data.dailyBreakdown.forEach((d) => {
      csv += `${new Date(d.date).toISOString().slice(0, 10)},${d.clicks},${d.conversions}\n`;
    });
    ...
    link.href = url;
    link.download = `sales-rep-${r.name.replace(/\s+/g, '-')}-${new Date().toISOString().split('T')[0]}.csv`;
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-4 text-sm text-gray-500">
              ...
              {rep.lastLogin && <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" />Last login: {formatFullDate(rep.lastLogin)}</span>}
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleExportCsv} className="flex items-center gap-2 rounded-lg bg-[var(--brand-primary)] px-4 py-2 text-sm font-medium text-white hover:bg-[#3d8a66]">
          ...
          {preset === 'custom' && (
            <>
              <div>
                ...
                <input type="date" value={customStart} onChange={(e) => setCustomStart(e.target.value)} className="rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[var(--brand-primary)] focus:outline-none" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-500">End</label>
                <input type="date" value={customEnd} onChange={(e) => setCustomEnd(e.target.value)} className="rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[var(--brand-primary)] focus:outline-none" />
              </div>
            </>
          )}
        </div>
        {data.dateRange && <div className="mt-2 text-xs text-gray-500">Showing: {formatDateRange(data.dateRange.startDate, data.dateRange.endDate)}</div>}
      </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-xl bg-white p-5 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-2xl font-bold text-gray-900">{stats.totalClicks.toLocaleString()}</p><p className="text-sm text-gray-500">Clicks</p></div>
            </div>
          </div>
          <div className="rounded-xl bg-white p-5 shadow-sm">
            <div className="flex items-center gap-3">
              <div className="rounded-lg bg-green-100 p-2 text-green-600"><TrendingUp className="h-5 w-5" /></div>
              <div><p className="text-2xl font-bold text-gray-900">{stats.totalConversions.toLocaleString()}</p><p className="text-sm text-gray-500">Conversions</p></div>
            </div>
          </div>
          <div className="rounded-xl bg-white p-5 shadow-sm">
            <div className="flex items-center gap-3">
              <div className="rounded-lg bg-yellow-100 p-2 text-yellow-600"><UserCheck className="h-5 w-5" /></div>
              <div><p className="text-2xl font-bold text-gray-900">{stats.patientsAssigned.toLocaleString()}</p><p className="text-sm text-gray-500">Patients Assigned</p></div>
            </div>
          </div>
          <div className="rounded-xl bg-white p-5 shadow-sm">
            <div className="flex items-center gap-3">
              <div className="rounded-lg bg-purple-100 p-2 text-purple-600"><Target className="h-5 w-5" /></div>
              <div><p className="text-2xl font-bold text-gray-900">{formatPercent(stats.conversionRate)}</p><p className="text-sm text-gray-500">Conv. Rate</p></div>
            </div>
          </div>
        </div>
              ...
              {data.dailyBreakdown.map((day, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-3">
                    <span className="w-14 flex-shrink-0 text-xs text-gray-500">{formatDate(day.date)}</span>
                    <div className="flex-1">
                      <div className="h-4 rounded-r bg-[var(--brand-primary)] transition-all" style={{ width: `${Math.max(w, 2)}%` }} />
                    </div>
                    <span className="w-10 text-right text-xs font-medium text-gray-700">{day.conversions}</span>
                    <span className="w-12 text-right text-xs text-blue-600">{day.clicks} cl.</span>
                  </div>
                );
              })}
            </div>
              ...
              {sortedCodes.map((code) => (
                <div key={code.id} className="rounded-lg border border-gray-100 p-3">
                  ...
                  <div className="mt-2 flex items-center gap-4 text-xs text-gray-500">
                    ...
                    <span>{formatPercent(code.conversionRate)} rate</span>
                  </div>
                  <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-gray-100">
                    <div className="h-full rounded-full bg-[var(--brand-primary)]" style={{ width: `${Math.min(code.conversionRate * 2, 100)}%` }} />
                  </div>
                </div>
              ))}
            </div>
