...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const providerInfo = {
      ...
      primarySpecialty:
        result.taxonomies?.find((t: any) => t.primary)?.desc || result.taxonomies?.[0]?.desc || '',
      // License info from taxonomy
      licenseNumber:
        result.taxonomies?.find((t: any) => t.primary)?.license ||
        result.taxonomies?.[0]?.license ||
        '',
      licenseState:
        result.taxonomies?.find((t: any) => t.primary)?.state ||
        result.taxonomies?.[0]?.state ||
        '',
      // Addresses
      addresses:
        result.addresses?.map((a: any) => ({
    ...
    return NextResponse.json({ error: error.message || 'Failed to lookup NPI' }, { status: 500 });
  }
}
