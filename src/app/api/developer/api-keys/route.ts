...
export const POST = withAuth(async (req: NextRequest, user) => {
  try {
    ...
    const prefix = process.env.NODE_ENV !== "development" && process.env.NODE_ENV !== "test" ? 'sk_live_' : 'sk_test_';
    ...
    // Create audit log
    await (prisma.userAuditLog as any).create({ data: {
...
export const DELETE = withAuth(async (req: NextRequest, user) => {
  try {
    ...
    logger.info(`API key ${keyId} revoked by ${user.email}`);
  ...
  try {
    ...
    switch (action) {
      case 'regenerate':
        ...
        logger.info(`API key ${id} regenerated by ${user.email}`);
        break;
      ...
      case 'update':
        ...
        logger.info(`API key ${id} updated by ${user.email}`);
        break;
