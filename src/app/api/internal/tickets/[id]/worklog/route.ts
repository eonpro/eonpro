...
async function getHandler(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const totalWorkTime = workLogs.reduce(
      (total: number, log: { duration: number | null }) => total + (log.duration || 0),
      0
    );
...
async function postHandler(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { userId, action, duration, description, isInternal = true, metadata } = body;
