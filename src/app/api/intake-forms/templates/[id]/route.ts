...
export const GET = withProviderAuth(
  async (req: NextRequest, user, context: RouteParams) => {
    try {
      ...
      const isAdmin = (user.role as string) === "admin" || (user.role as string) === "admin" || (user.role as string) === "admin";
      const isOwner = !template.providerId || template.providerId === user.providerId || template.providerId === 1; // Allow provider ID 1 for testing
 ...
 */
export const PUT = withProviderAuth(
  async (req: NextRequest, user, context: RouteParams) => {
    try {
      const resolvedParams = await context.params;
      const templateId = parseInt(resolvedParams.id);
