    ...
    try {
      ...
      const safeQuery = async <T>(fn: () => Promise<T>, fallback: T): Promise<T> => {
        try {
          ...
          logger.error('[HealthScore] DB query failed', {
      ...
      if (weightLogs.length > 0) {
        ...
        const firstWeight = await safeQuery(
          () =>
            prisma.patientWeightLog.findFirst({
              where: { patientId: pid },
      ...
      metrics.push({
        ...
        lastUpdated:
          weightLogs.length > 0 ? formatLastUpdated(weightLogs[0].recordedAt) : 'No data',
      });
      ...
      // Streak Score (0-100)
      const weightStreak = streaks.find(
