...
function detectCategory(
  description: string | null,
  metadata: Record<string, string>
): { category: TransactionCategory; label: string; productName?: string } {
  ...
  if (
    ...
    desc.includes('rybelsus')
  ) {
    return {
      category: 'semaglutide',
      label: 'Semaglutide',
      productName: productName || 'Semaglutide',
    };
  }
  ...
  if (
    ...
    return {
      ...
      label: 'Tirzepatide',
      productName: productName || 'Tirzepatide',
    };
  }
  ...
  if (
    desc.includes('new patient') ||
    desc.includes('initial') ||
    desc.includes('intake') ||
    desc.includes('first visit') ||
    metadata?.new_patient === 'true'
  ) {
    ...
    try {
      ...
      if (type === 'all' || type === 'charges') {
        ...
        for (const charge of charges.data) {
          ...
          const { category, label, productName } = detectCategory(
            charge.description,
            charge.metadata || {}
          );
      ...
      if (type === 'all' || type === 'refunds') {
        ...
        for (const refund of refunds.data) {
          ...
          const { category, label, productName } = detectCategory(
            charge?.description || refund.reason || null,
            refund.metadata || {}
          );
      ...
      const successfulCharges = transactions.filter(
      ...
      const lastChargeId = transactions.filter((t) => t.type === 'charge').slice(-1)[0]?.id;
