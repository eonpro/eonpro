  ...
  try {
    ...
    const subscriptions: SubscriptionInfo[] = rawSubscriptions
      ...
      : singleSubscription
        ? [singleSubscription]
        : [];
    const lineItems = paymentMeta.lineItems as
      | Array<{ description: string; amount: number; planId?: string }>
      | undefined;
      ...
      try {
        ...
        if (existingPm) {
          ...
          const brand = (pm as Stripe.PaymentMethod).card?.brand
            ? String((pm as Stripe.PaymentMethod).card?.brand)
                .charAt(0)
                .toUpperCase() + String((pm as Stripe.PaymentMethod).card?.brand).slice(1)
            : 'Unknown';
      ...
      async (tx) => {
        ...
        if (subscriptions.length > 0 && stripeStatus === PaymentStatus.SUCCEEDED) {
          ...
          for (const sub of subscriptions) {
            ...
            const totalMonths =
              sub.intervalCount * (sub.interval === 'year' ? 12 : sub.interval === 'month' ? 1 : 1);
            ...
            const subscriptionRecurringAmount =
              sub.discountMode === 'first_only' && sub.catalogAmountCents
