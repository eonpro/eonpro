...
import { FileCategory, FileAccessLevel, isS3Enabled, S3_ERRORS } from '@/lib/integrations/aws/s3Config';
  ...
  try {
    ...
    if (!isFeatureEnabled('AWS_S3_STORAGE')) {
      ...
      return NextResponse.json([
        ...
        if (patientId && 'patientId' in doc && doc.patientId !== parseInt(patientId)) return false;
        return true;
      }));
    }
    ...
    const documents = files.map((file, index) => {
      ...
      const category = parts.includes('patients') && parts.length > 2 
    ...
    return NextResponse.json(
      { error: errorMessage || 'Failed to list files' },
      { status: 500 }
    );
  }
}