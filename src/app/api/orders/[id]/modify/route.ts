  ...
  try {
    ...
    if (order.lifefileOrderId && order.clinic?.lifefileEnabled) {
      try {
        ...
        logger.info(`[ORDER MODIFY] Calling Lifefile modify API for order ${order.lifefileOrderId}`);
        ...
        if (modifications.shipping) {
          try {
            ...
            logger.info(`[ORDER MODIFY] Lifefile shipping update response:`, lifefileModifyResponse);
