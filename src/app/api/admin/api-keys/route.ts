...
export const PUT = withAuth(
  async (req: NextRequest, user) => {
    try {
      ...
      logger.info('API key updated', { keyName: existing.name, userId: user.id });
  ...
  async (req: NextRequest, user) => {
    try {
      ...
      if (permanent) {
        ...
        logger.warn('API key permanently deleted', { keyName: apiKey.name, userId: user.id });
        ...
        logger.info('API key revoked', { keyName: apiKey.name, userId: user.id });
