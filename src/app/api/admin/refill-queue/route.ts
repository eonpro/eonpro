...
export const GET = withAuth(async (req: NextRequest, user: AuthUser) => {
  try {
    ...
    const status = statusParam
      ? statusParam.split(',') as RefillStatus[]
      : undefined;
    ...
    logger.error('[Admin RefillQueue] Error listing refills', error);
    return NextResponse.json(
      { error: 'Failed to list refills' },
      { status: 500 }
    );
  }
...
export const POST = withAuth(async (req: NextRequest, user: AuthUser) => {
  try {
    ...
    if (action !== 'process_due') {
      return NextResponse.json(
        { error: 'Invalid action. Use "process_due" to process due refills.' },
        { status: 400 }
      );
    }
    ...
    logger.error('[Admin RefillQueue] Error processing due refills', error);
    return NextResponse.json(
      { error: 'Failed to process due refills' },
      { status: 500 }
    );
  }
}, { roles: ['super_admin', 'admin'] });
