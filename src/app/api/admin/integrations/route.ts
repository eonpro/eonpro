...
export const POST = withAuth(
  async (req: NextRequest, user) => {
    try {
      ...
      logger.info('Integration configured', { provider: validated.provider, userId: user.id });
...
export const PUT = withAuth(
  async (req: NextRequest, user) => {
    try {
      ...
      logger.info('Integration updated', { provider: existing.provider, userId: user.id });
...
export const DELETE = withAuth(
  async (req: NextRequest, user) => {
    try {
      ...
      logger.warn('Integration deleted', { provider: integration.provider, userId: user.id });
