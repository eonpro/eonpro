...
export const GET = withAuth(async (req: NextRequest, user: AuthUser, context?: RouteContext) => {
  try {
    ...
    // Format standings
    const standings = competition.entries.map((entry: typeof competition.entries[number], index: number) => ({
...
export const DELETE = withAuth(async (req: NextRequest, user: AuthUser, context?: RouteContext) => {
  try {
    ...
    if (existing._count.entries > 0 || existing.status === 'ACTIVE' || existing.status === 'COMPLETED') {
