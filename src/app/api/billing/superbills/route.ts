...
export const GET = withProviderAuth(async (req: NextRequest, user) => {
  try {
    ...
    if (superbillId) {
      ...
      if (!superbill) {
        return NextResponse.json({ error: 'Superbill not found' }, { status: 404 });
      }
    ...
    return NextResponse.json({ error: 'Failed to fetch superbills' }, { status: 500 });
  }
});
...
export const POST = withProviderAuth(async (req: NextRequest, user) => {
  try {
    ...
    return NextResponse.json({ error: 'Failed to create superbill' }, { status: 500 });
  }
});
...
export const PATCH = withProviderAuth(async (req: NextRequest, user) => {
  try {
    ...
    if (!superbillId || !action) {
      return NextResponse.json({ error: 'superbillId and action are required' }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: 'Failed to update superbill' }, { status: 500 });
  }
});
