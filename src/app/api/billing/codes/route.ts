...
export const GET = withAuth(async (req: NextRequest, user) => {
  try {
    ...
    if (!codeType) {
      return NextResponse.json({ error: 'codeType is required (CPT or ICD10)' }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: 'Failed to search billing codes' }, { status: 500 });
  }
});
...
export const POST = withAdminAuth(async (req: NextRequest, user) => {
  try {
    ...
    return NextResponse.json({ error: 'Failed to create billing code' }, { status: 500 });
  }
});
...
export const DELETE = withAdminAuth(async (req: NextRequest, user) => {
  try {
    ...
    return NextResponse.json({ error: 'Failed to delete billing code' }, { status: 500 });
  }
});
