  ...
  try {
    ...
    await prisma.userAuditLog.create({ data: {
        ...
        details: {
          ...
          oldValue: settingDef.sensitive ? '***' : getSettingValue(settingId, settingDef.defaultValue),
    ...
    logger.info(`Setting ${settingId} updated by ${user.email}`);
  ...
  try {
    ...
    switch (integration) {
      ...
      case 'stripe':
        ...
        try {
          const stripe = require('stripe')(settings.secret_key);
      ...
      case 'sendgrid':
        ...
        try {
          ...
          testResult = {
            success: response.ok,
            message: response.ok ? 'SendGrid connection successful' : `Connection failed: ${response.status}`,
          };
        } catch (error: any) {
          testResult = {
            success: false,
      ...
      case 'openai':
        ...
        try {
          ...
          testResult = {
            success: response.ok,
            message: response.ok ? 'OpenAI connection successful' : `Connection failed: ${response.status}`,
          };
        } catch (error: any) {
          testResult = {
            success: false,
    ...
    // Log test attempt
    logger.info(`Integration test for ${integration} by ${user.email}: ${testResult.success}`);
