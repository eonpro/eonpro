...
export async function POST(req: NextRequest) {
  ...
  const authResult = {
    ...
    headerUsed: req.headers.get('x-webhook-secret')
      ? 'x-webhook-secret'
      : req.headers.get('x-api-key')
        ? 'x-api-key'
  ...
  if (!authResult.valid) {
    ...
    const debugInfo = {
      ...
      providedCharAtDiff:
        firstDiffIndex >= 0
          ? `'${p[firstDiffIndex]}' (code: ${p.charCodeAt(firstDiffIndex)})`
          : 'N/A',
      configuredCharAtDiff:
        firstDiffIndex >= 0
          ? `'${c[firstDiffIndex]}' (code: ${c.charCodeAt(firstDiffIndex)})`
          : 'N/A',
      providedAround:
        firstDiffIndex >= 0
          ? p.substring(Math.max(0, firstDiffIndex - 3), firstDiffIndex + 4)
          : 'N/A',
      configuredAround:
        firstDiffIndex >= 0
          ? c.substring(Math.max(0, firstDiffIndex - 3), firstDiffIndex + 4)
          : 'N/A',
    };
  ...
  const parsed = text ? safeParseJsonString<Record<string, unknown>>(text) : null;
  const payload: Record<string, unknown> =
    parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed : {};
  const parseError: string | null =
    text && (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed))
      ? 'Invalid JSON'
      : null;
  ...
  const qualified = String(
    payload.qualified || (payload.data as any)?.qualified || (isPartial ? 'Pending' : 'Yes')
  );
...
function generateHints(normalized: any, warnings: string[], format: string): string[] {
  const hints: string[] = [];
...
// GET - Return documentation (dev only; 404 in production per B5)
