/**
 * Overnight Weight Loss — Lifefile Shipping Webhook
 *
 * Receives shipping/tracking updates from Lifefile for Overnight Weight Loss prescriptions.
 ...
 * Configure at: /super-admin/clinics/9 -> Pharmacy tab -> Inbound Webhook Settings
 *
 * Example curl for LifeFile to call:
 *
 * curl -X POST https://overnight.eonpro.io/api/webhooks/overnight-shipping \
...
import { normalizeLifefilePayload } from '@/lib/shipping/normalize-lifefile-payload';
import crypto from 'crypto';

const OVERNIGHT_SUBDOMAIN = 'overnight';
const LOG_PREFIX = '[OVERNIGHT SHIPPING]';
const BRAND = 'Overnight Weight Loss';
...
async function verifyBasicAuth(
  ...
  if (!expectedPassword) {
    logger.error(`${LOG_PREFIX} No inbound webhook password configured for clinic`);
    return { success: false, diagnostics: { reason: 'no_password_configured' } };
  }

  if (!authHeader) {
    logger.error(`${LOG_PREFIX} Missing Authorization header`);
  ...
  try {
    ...
    if (!usernameMatch) {
      logger.error(`${LOG_PREFIX} Auth failed: username "${username}" not recognized`);
      return {
        success: false,
        diagnostics: {
    ...
    if (passwordMatch) {
      logger.info(`${LOG_PREFIX} Authentication successful (username: ${username})`);
      return { success: true };
    }

    logger.error(`${LOG_PREFIX} Auth failed: password mismatch`, {
      usernameReceived: username,
      passwordLenReceived: password.length,
      passwordLenExpected: expectedPassword.length,
    });
    ...
    logger.error(`${LOG_PREFIX} Error parsing auth header:`, error);
...
function parseDate(dateStr: string | undefined): Date | undefined {
  ...
  try {
    ...
    logger.warn(`${LOG_PREFIX} Date parse failed`, {
...
/**
 * POST /api/webhooks/overnight-shipping
 * Receives shipping updates from Lifefile for Overnight Weight Loss (clinic 9)
 */
export async function POST(req: NextRequest) {
  const startTime = Date.now();
  const requestId = `overnight-ship-${Date.now()}`;

  let webhookLogData: any = {
    endpoint: '/api/webhooks/overnight-shipping',
    method: 'POST',
  ...
  try {
    ...
    logger.info(`${LOG_PREFIX} New webhook request - ${requestId}`);
    logger.info(`${LOG_PREFIX} Time: ${new Date().toISOString()}`);
    ...
    const clinic = await basePrisma.clinic.findFirst({
      where: { subdomain: OVERNIGHT_SUBDOMAIN },
      select: {
        id: true,
        name: true,
    ...
    if (!clinic) {
      logger.error(`${LOG_PREFIX} Overnight clinic not found (subdomain: ${OVERNIGHT_SUBDOMAIN})`);
      return NextResponse.json({ error: 'Clinic not found' }, { status: 500 });
    }

    if (!clinic.lifefileInboundEnabled) {
      logger.warn(`${LOG_PREFIX} Inbound webhook not enabled for clinic`);
    ...
    const writeWebhookLog = () =>
      runWithClinicContext(clinic.id, () =>
        ...
        logger.warn(`${LOG_PREFIX} Failed to persist webhook log`, {
    ...
    const data = normalizeLifefilePayload(rawPayload, 'OVERNIGHT SHIPPING');
    if (!data) {
      webhookLogData.status = WebhookStatus.INVALID_PAYLOAD;
      webhookLogData.statusCode = 400;
    ...
    logger.info(
      `${LOG_PREFIX} Processing shipment - Order: ${data.orderId}, Tracking: ${data.trackingNumber}`
    );
    ...
    return runWithClinicContext(clinic.id, async () => {
      ...
      if (!result) {
        logger.warn(`${LOG_PREFIX} No match for order ${data.orderId} — storing as unmatched`);
        ...
        logger.info(`${LOG_PREFIX} Stored unmatched record ${unmatchedShipping.id}`);
      ...
      logger.info(`${LOG_PREFIX} Matched via strategy: ${matchStrategy}`);
      ...
      if (existingUpdate) {
        ...
        logger.info(`${LOG_PREFIX} Updated existing shipping record ${existingUpdate.id}`);
      } else {
        ...
        logger.info(`${LOG_PREFIX} Created new shipping record ${shippingUpdate.id}`);
        ...
        sendTrackingNotificationSMS({
          ...
          logger.warn(`${LOG_PREFIX} Tracking SMS failed (non-blocking)`, {
            error: err instanceof Error ? err.message : String(err),
            patientId: patient.id,
          });
        });
      ...
      logger.info(`${LOG_PREFIX} Processing completed in ${processingTime}ms`);
      logger.info('='.repeat(60));
    ...
    logger.error(`${LOG_PREFIX} Error processing webhook:`, {
      error: errorMessage,
    });
    ...
    if (webhookLogData.clinicId) {
      await runWithClinicContext(webhookLogData.clinicId, () =>
        ...
        logger.warn(`${LOG_PREFIX} Failed to log webhook error:`, {
          error: dbError instanceof Error ? dbError.message : String(dbError),
        });
      });
    }

    return handleApiError(error, { route: 'POST /api/webhooks/overnight-shipping' });
  }
}

/**
 * GET /api/webhooks/overnight-shipping
