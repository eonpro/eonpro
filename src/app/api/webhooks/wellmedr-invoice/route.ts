...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const errMsg =
      err instanceof Error ? (err instanceof Error ? err.message : String(err)) : 'Unknown error';
  ...
  return runWithClinicContext(clinicId, async () => {
    ...
    if (existingIdempotencyRecord) {
      logger.info(
        `[WELLMEDR-INVOICE ${requestId}] Duplicate request detected, returning cached result`,
        {
          idempotencyKey: idempotencyKey.substring(0, 40) + '...',
          originalRequestId: existingIdempotencyRecord.responseBody ? 'exists' : 'none',
        }
      );
      const cachedResponse = existingIdempotencyRecord.responseBody as Record<
        string,
        unknown
      > | null;
    ...
    if (payload.submission_id) {
      ...
      const existingLogical = await prisma.idempotencyRecord
        .findUnique({
      ...
      if (existingLogical) {
        logger.info(
          `[WELLMEDR-INVOICE ${requestId}] Logical duplicate detected (same submission_id)`,
          {
    ...
    if (INVALID_EMAILS.includes(emailLower) || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailLower)) {
      logger.warn(
        `[WELLMEDR-INVOICE ${requestId}] Invalid/placeholder email rejected: ${emailLower}`
      );
      return NextResponse.json(
        { error: `Invalid email: "${emailLower}" looks like a column header or placeholder` },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (directMatch) {
        ...
        if (directEmail === email) {
          ...
          logger.info(
            `[WELLMEDR-INVOICE ${requestId}] ✓ Patient matched by searchIndex (fast path) in ${Date.now() - matchStart}ms`,
            {
      ...
      if (!patient) {
        ...
        for (const candidate of patientsWithoutIndex) {
          ...
          if (candEmail === email) {
            ...
            logger.info(
              `[WELLMEDR-INVOICE ${requestId}] ✓ Patient matched by null-index scan in ${Date.now() - scanStart}ms`,
              {
                patientId: patient.id,
      ...
      if (!patient) {
        ...
        for (const candidate of emailResults.data) {
          const candEmail = safeDecrypt((candidate as { email?: string | null }).email)
            ?.toLowerCase()
            .trim();
          if (candEmail === email) {
            patient = candidate as WellMedRPatient;
            ...
            logger.info(
              `[WELLMEDR-INVOICE ${requestId}] ✓ Patient matched by email (PHISearch) in ${Date.now() - phiStart}ms`,
              {
                patientId: patient.id,
      ...
      if (!patient && paymentName) {
        ...
        if (nameResults.data.length === 1) {
          ...
          logger.info(
            `[WELLMEDR-INVOICE ${requestId}] ✓ Patient matched by name in ${Date.now() - nameStart}ms`,
            {
              patientId: patient.id,
      ...
      logger.info(
        `[WELLMEDR-INVOICE ${requestId}] Patient matching completed in ${Date.now() - matchStart}ms`,
        {
      ...
      if (!patient) {
        logger.warn(
          `[WELLMEDR-INVOICE ${requestId}] Patient NOT found — auto-creating stub patient`,
          {
            searchedEmail: email,
        ...
        logger.info(
          `[WELLMEDR-INVOICE ${requestId}] Stub patient data prepared (deferred to transaction)`,
          {
      ...
      const errMsg =
        err instanceof Error ? (err instanceof Error ? err.message : String(err)) : 'Unknown error';
      logger.error(`[WELLMEDR-INVOICE ${requestId}] Error finding/creating patient:`, {
        error: errMsg,
      });
    ...
    const decryptedPatientName =
      `${safeDecrypt(verifiedPatient.firstName) || ''} ${safeDecrypt(verifiedPatient.lastName) || ''}`
        .trim()
        .toLowerCase();
    const paymentNameLower = paymentName.toLowerCase();
    if (paymentName && decryptedPatientName && paymentNameLower !== decryptedPatientName) {
      logger.info(
        `[WELLMEDR-INVOICE ${requestId}] Payment name differs from patient (intake) name — this is normal`,
        {
          paymentName,
    ...
    if (!wasAutoCreated) {
      try {
        ...
        logger.warn(`[WELLMEDR-INVOICE ${requestId}] Error checking for duplicate:`, {
          error: err,
        });
      }
    }
    ...
    if (isPlanOnly && medicationType) {
      ...
      const altMed = payload.medication || payload.treatment || payload.product_name || '';
      if (
        altMed &&
        /tirzepatide|semaglutide|mounjaro|zepbound|ozempic|wegovy/i.test(String(altMed))
      ) {
    ...
    if (addonIds.length === 0 && payload.selected_addons) {
      try {
        const fromPayload =
          typeof payload.selected_addons === 'string'
    ...
    try {
      ...
      // Prevents orphaned stub patients if invoice creation fails.
      const { txPatient, invoice } = await prisma.$transaction(
        async (tx) => {
          ...
          const createdInvoice = await tx.invoice.create({
            data: {
              ...
              metadata: {
                ...
                previousGlp1Details:
                  payload.previous_glp1_details || payload.previousGlp1Details || '',
      ...
      if (invoiceHasPlanOnly) {
        try {
          ...
          const docJson = intakeDoc
            ? ((await readIntakeData(intakeDoc)) as Record<string, unknown> | null)
            : null;
          ...
          if (docJson && typeof docJson === 'object') {
            ...
            if (!preferredMedication && docJson.answers && Array.isArray(docJson.answers)) {
              for (const a of docJson.answers) {
                const key = String(a.question || a.field || a.id || a.label || '')
                  .toLowerCase()
                  .replace(/[-_\s]/g, '');
          ...
          if (!preferredMedication && amountInCents > 0) {
            ...
            const effectiveMonths =
              planMonths || (prodMonthMatch ? parseInt(prodMonthMatch[1], 10) : 0);
            ...
            if (threshold) {
              ...
              for (const kp of knownPrices) {
                if (Math.abs(dollars - kp.price) / kp.price < 0.1) {
                  preferredMedication = kp.med;
                  logger.info(
                    `[WELLMEDR-INVOICE ${requestId}] ✓ Derived medication from exact price match`,
                    {
                      preferredMedication,
                      dollars,
                      matchedPrice: kp.price,
                    }
                  );
                  break;
                }
          ...
          if (preferredMedication) {
            ...
            logger.info(
              `[WELLMEDR-INVOICE ${requestId}] ✓ Stored preferred medication in invoice metadata`,
              {
                preferredMedication,
                patientId: verifiedPatient.id,
                invoiceId: invoice.id,
            ...
            logger.info(
              `[WELLMEDR-INVOICE ${requestId}] Could not determine preferred medication`,
              {
                patientId: verifiedPatient.id,
                invoiceId: invoice.id,
                product,
          ...
          logger.warn(`[WELLMEDR-INVOICE ${requestId}] Medication extraction failed (non-fatal)`, {
            error:
              medExtractErr instanceof Error
                ? medExtractErr instanceof Error
                  ? medExtractErr.message
                  : String(medExtractErr)
                : String(medExtractErr),
          });
        }
      }
      ...
      const idempotencyRecords: Array<{
        key: string;
        resource: string;
