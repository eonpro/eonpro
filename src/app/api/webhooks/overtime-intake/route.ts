...
import { normalizeOvertimePayload, extractPromoCode, isCheckoutComplete, detectTreatmentType } from "@/lib/overtime/intakeNormalizer";
...
if (!EXPECTED_OVERTIME_CLINIC_ID) {
  logger.warn('[OVERTIME-INTAKE] OVERTIME_CLINIC_ID env var not set - will use dynamic lookup only');
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    const overtimeClinic = await withRetry<{ id: number; name: string; subdomain: string | null } | null>(() => prisma.clinic.findFirst({
      where: {
        OR: [
    ...
    if (EXPECTED_OVERTIME_CLINIC_ID && clinicId !== EXPECTED_OVERTIME_CLINIC_ID) {
      ...
      recordError("overtime-intake", `SECURITY: Clinic ID mismatch - expected ${EXPECTED_OVERTIME_CLINIC_ID}, got ${clinicId}`, {
        requestId,
        expected: EXPECTED_OVERTIME_CLINIC_ID,
        found: clinicId,
      });
    ...
    logger.info(`[OVERTIME-INTAKE ${requestId}] ✓ CLINIC VERIFIED: ID=${clinicId}, Name="${overtimeClinic.name}", Subdomain="${overtimeClinic.subdomain}"`);
  } catch (err) {
    const errMsg = err instanceof Error ? err.message : 'Unknown error';
    logger.error(`[OVERTIME-INTAKE ${requestId}] Database error finding clinic:`, { error: errMsg });
    recordError("overtime-intake", `Database error: ${err instanceof Error ? err.message : 'Unknown'}`, { requestId });
    ...
    if (isDLQConfigured()) {
      try {
        ...
        logger.error(`[OVERTIME-INTAKE ${requestId}] Failed to queue to DLQ:`, { error: dlqErrMsg });
      }
    }
  ...
  logger.info(`[OVERTIME-INTAKE ${requestId}] Treatment Type: ${treatmentLabel} (${treatmentType})`);
  ...
  const buildNotes = (existing: string | null | undefined) => {
    ...
    parts.push(`[${new Date().toISOString()}] ${isPartialSubmission ? "PARTIAL" : "COMPLETE"} - ${treatmentLabel}: ${normalized.submissionId}`);
    return parts.join("\n");
  };
  ...
  try {
    ...
    if (lookupConditions.length > 0) {
      existingPatient = await withRetry(() => prisma.patient.findFirst({
        where: {
          clinicId: clinicId,
          OR: lookupConditions,
        },
    ...
    if (existingPatient) {
      ...
      logger.info(`[OVERTIME-INTAKE ${requestId}] ✓ Updated patient: ${patient.id} → OVERTIME CLINIC ONLY (clinicId=${clinicId})`);
    } else {
      ...
      while (!created && retryCount < MAX_RETRIES) {
        try {
          ...
          isNewPatient = true;
          created = true;
          logger.info(`[OVERTIME-INTAKE ${requestId}] ✓ Created patient: ${patient.id} (${patient.patientId}) → OVERTIME CLINIC ONLY (clinicId=${clinicId})`);
        } catch (createErr: any) {
          if (createErr?.code === 'P2002' && createErr?.meta?.target?.includes('patientId')) {
            retryCount++;
            logger.warn(`[OVERTIME-INTAKE ${requestId}] PatientId conflict, retrying (${retryCount}/${MAX_RETRIES})...`);
            ...
            if (retryCount >= 3 && lookupConditions.length > 0) {
              ...
              if (refetchPatient) {
                ...
                created = true;
                logger.info(`[OVERTIME-INTAKE ${requestId}] ✓ Found and updated patient on retry: ${patient.id}`);
              }
            }
          } else {
            throw createErr;
  ...
  try {
    ...
    const errMsg = err instanceof Error ? err.message : 'Unknown error';
    logger.warn(`[OVERTIME-INTAKE ${requestId}] PDF generation failed (continuing):`, { error: errMsg });
  ...
  if (pdfContent) {
    try {
      ...
      const errMsg = err instanceof Error ? err.message : 'Unknown error';
      logger.warn(`[OVERTIME-INTAKE ${requestId}] PDF preparation failed (continuing):`, { error: errMsg });
  ...
  if (pdfContent && stored) {
    try {
      ...
      const errMsg = err instanceof Error ? err.message : 'Unknown error';
      logger.warn(`[OVERTIME-INTAKE ${requestId}] S3 upload failed (continuing):`, { error: errMsg });
  ...
  if (!isPartialSubmission && patientDocument) {
    try {
      ...
      const errMsg = err instanceof Error ? err.message : 'Unknown error';
      logger.warn(`[OVERTIME-INTAKE ${requestId}] SOAP generation failed (non-fatal):`, { error: errMsg });
