...
const shippingPayloadSchema = z.object({
  ...
  // Optional fields
  brand: z.string().optional().default('EONpro (OT)'),
...
async function findPatient(
  ...
  if (patient) {
    ...
    if (recentOrder) {
      logger.info(
        `[OT SHIPPING] Found recent order ${recentOrder.id} for patient ${patient.id} without tracking`
      );
    ...
    if (recentOrderWithinMonth) {
      logger.info(
        `[OT SHIPPING] Found recent order ${recentOrderWithinMonth.id} within 30 days for patient ${patient.id}`
      );
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!parseResult.success) {
      ...
      logger.error('[OT SHIPPING] Validation failed:', errors);
    ...
    if (!result) {
      logger.warn(`[OT SHIPPING] Patient/Order not found for order ${data.orderId}`);
