...
async function handler(req: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const pendingCommissions = await prisma.affiliateCommissionEvent.count({
      where: { status: 'PENDING' },
    });
    const approvedCommissions = await prisma.affiliateCommissionEvent.count({
      where: { status: 'APPROVED' },
    });
    const paidCommissions = await prisma.affiliateCommissionEvent.count({
      where: { status: 'PAID' },
    });
    ...
    const activePlanCount = await prisma.affiliateCommissionPlan.count({
      where: { isActive: true },
    });
