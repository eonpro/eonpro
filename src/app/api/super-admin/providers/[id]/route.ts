...
export const PUT = withSuperAdminAuth(
  async (req: NextRequest, user: AuthUser, context: { params: Promise<{ id: string }> }) => {
    try {
      ...
      if (
        typeof body.isEonproProvider === 'boolean' &&
        body.isEonproProvider !== existing.isEonproProvider
      ) {
        ...
        return NextResponse.json(
          {
            ...
            message:
              'This provider has associated orders, appointments, or SOAP notes. Use force=true to delete anyway.',
