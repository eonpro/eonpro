 ...
 */
export const GET = withAuth(async (req: NextRequest, user: AuthUser, context?: RouteContext) => {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch commission plan' },
      { status: 500 }
    );
  }
}, { roles: ['super_admin'] });
 ...
 */
export const PUT = withAuth(async (req: NextRequest, user: AuthUser, context?: RouteContext) => {
  try {
    const { id } = await context!.params;
    const planId = parseInt(id);
    ...
    return NextResponse.json(
      { error: 'Failed to update commission plan' },
      { status: 500 }
    );
  }
}, { roles: ['super_admin'] });
 ...
 */
export const PATCH = withAuth(async (req: NextRequest, user: AuthUser, context?: RouteContext) => {
  try {
    const { id } = await context!.params;
    const planId = parseInt(id);
 ...
 */
export const DELETE = withAuth(async (req: NextRequest, user: AuthUser, context?: RouteContext) => {
  try {
    const { id } = await context!.params;
    const planId = parseInt(id);
    ...
    return NextResponse.json(
      { error: 'Failed to delete commission plan' },
      { status: 500 }
    );
  }
}, { roles: ['super_admin'] });
