  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          hint: 'Your account is not linked to a provider profile. Ask an admin to link your user to a provider or ensure a provider exists with your email.',
        },
        { status: 404 }
      );
    }
    ...
    switch (action) {
      ...
      case 'events':
        ...
        const events = await fetchAllCalendarEvents(provider.id, startDate, endDate);
  ...
  try {
    ...
    switch (action) {
      case 'connect':
        ...
        let authResult: Awaited<ReturnType<typeof getCalendarAuthUrl>>;
        try {
          authResult = await getCalendarAuthUrl(
          ...
          if (
            ...
            msg.includes('Google Calendar OAuth is not configured')
          ) {
            return NextResponse.json(
              {
                error:
                  'Google Calendar is not configured for this environment. Contact your administrator to set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET.',
        ...
        // Apple returns setup instructions (iCal subscription), not an OAuth URL
        if (
          typeof authResult === 'object' &&
          authResult !== null &&
          'type' in authResult &&
          authResult.type === 'setup' &&
          'setup' in authResult
        ) {
          return NextResponse.json({ success: true, setup: authResult.setup });
        }
        ...
        return NextResponse.json({ success: true, authUrl: authResult as string });

      case 'sync':
        // Trigger calendar sync
        const syncParsed = syncSchema.safeParse(body);

        const syncResult = await syncAllCalendars(provider.id, provider.clinicId || 0);
  ...
  try {
    ...
    if (!providerParam) {
      try {
        ...
        if (parsed.success) providerParam = parsed.data.provider;
      } catch {
    ...
    const result = await disconnectCalendar(provider.id, providerParam as CalendarProvider);
