    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800">
        ...
        {loading ? (
          ...
          health && (
            <>
                ...
                className={`mb-8 rounded-2xl p-8 ${
                  health.status === 'healthy'
                    ...
                    : health.status === 'degraded'
                      ? 'border border-amber-700 bg-amber-900/30'
                      : 'border border-red-700 bg-red-900/30'
                }`}
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`h-4 w-4 rounded-full ${getStatusColor(health.status)} animate-pulse`}
                  />
                  <div>
              ...
              {/* Services Status */}
              <div className="mb-8 overflow-hidden rounded-xl border border-slate-700 bg-slate-800/50">
                ...
                <div className="divide-y divide-slate-700">
                  ...
                  />
                  <ServiceRow name="Database" status={health.database as any} />
                  <ServiceRow name="Authentication" status={health.status} />
                  <ServiceRow name="API Services" status={health.status} />
                </div>
              </div>
              ...
              <div className="mb-8 rounded-xl border border-slate-700 bg-slate-800/50 p-6">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    <p className="mb-1 text-sm text-slate-400">Response Time</p>
                    <p className="text-2xl font-bold text-white">{health.responseTime}ms</p>
                  </div>
                  <div className="h-16 w-px bg-slate-700" />
                  <div className="flex-1">
                    <p className="mb-1 text-sm text-slate-400">Uptime (30 days)</p>
                  ...
                  <div className="p-4">
                    <div className="flex gap-2">
                      {history.map((h, i) => (
                        <div
                          key={i}
                          className={`h-8 w-8 rounded ${getStatusColor(h.status)} opacity-${100 - i * 10}`}
      ...
      <footer className="mt-12 border-t border-slate-700">
        <div className="mx-auto max-w-4xl px-6 py-6 text-center text-sm text-slate-500">
          ...
          <p className="mt-1">
            Need help? Contact{' '}
            <a href="mailto:support@eonpro.io" className="text-emerald-400 hover:underline">
              support@eonpro.io
            </a>
          </p>
        </div>
...
function ServiceRow({
  ...
  return (
    <div className="flex items-center justify-between px-6 py-4">
      ...
      <div className="flex items-center gap-3">
        {responseTime && <span className="text-sm text-slate-400">{responseTime}ms</span>}
        <span
