  ...
  return (
    ...
    >
      ...
      <div className="flex-1 px-6 lg:px-8">
        <div className="max-w-[480px] lg:max-w-[560px] mx-auto w-full py-6">

          {/* Back Button */}
          {currentStep > 1 && (
            <button
              onClick={handlePreviousStep}
              className="inline-block p-2 -ml-2 hover:bg-gray-100 rounded-lg transition-colors mb-2"
              aria-label={t.back}
            >
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="relative bg-white rounded-full p-0.5 shadow-sm border border-gray-200 inline-flex items-center">
              <button
                ...
                className={`relative z-10 px-2.5 py-1 rounded-full text-xs font-semibold transition-all duration-300 ${language === 'en' ? 'text-white' : 'text-gray-500'}`}
              >EN</button>
              <button
                onClick={() => setLanguage('es')}
                className={`relative z-10 px-2.5 py-1 rounded-full text-xs font-semibold transition-all duration-300 ${language === 'es' ? 'text-white' : 'text-gray-500'}`}
              >ES</button>
              <div className={`absolute top-0.5 bottom-0.5 rounded-full bg-[#413d3d] transition-all duration-300 ease-out ${language === 'en' ? 'left-0.5 right-[50%]' : 'left-[50%] right-0.5'}`} />
            </div>
          </div>

          {/* Status Banner */}
          {statusBanner && (
            <div className={`rounded-lg p-3 text-white text-center text-sm mb-6 ${statusBanner === 'success' ? 'bg-green-600' : 'bg-red-600'}`}>
          ...
          {currentStep === 1 && (
            <div className={`transition-all duration-300 ease-out ${isTransitioning ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
              <div className="space-y-2 mb-8">
                <h1 className="text-[1.875rem] lg:text-4xl font-[510] text-[#413d3d] leading-[1.1]">
                  {patientData.firstName
                    ? `${patientData.firstName}, ${t.chooseMedication}`
                    : t.chooseMedication}
                </h1>
                <p className="text-[15.5px] text-[#413d3d]/70 font-[350] leading-relaxed">{t.medicationSubtitle}</p>
              </div>
                    ...
                    <div className="flex items-center gap-4 w-full">
                      ...
                      <div className="flex-1 text-left">
                        <div className="flex items-center gap-2">
                          ...
                          {med.tag && (
                            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full text-white ${med.id === 'semaglutide' ? 'bg-[#413d3d]' : 'bg-orange-500'}`}>
                              {med.tag}
                            </span>
                          )}
                        </div>
                        <p className="text-sm text-[#413d3d]/70 mt-0.5">{med.strength} &middot; {med.description}</p>
                        <p className="text-sm font-semibold mt-1 text-[#413d3d]">
                          {t.asLowAs} ${med.lowestMonthlyPrice || med.plans[0].price}/{language === 'es' ? 'mes' : 'mo'}
                        </p>
                      </div>
                    </div>
                  </button>
          ...
          {currentStep === 2 && selectedMed && (
            <div className={`transition-all duration-300 ease-out ${isTransitioning ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
              <div className="space-y-2 mb-8">
                ...
                <p className="text-[15.5px] text-[#413d3d]/70 font-[350] leading-relaxed">{t.planSubtitle}</p>
              </div>

              {/* Selected Medication Preview */}
              <div className="flex items-center gap-3 p-4 bg-[#f5ecd8] rounded-2xl mb-6">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                ...
                />
                <div>
                  <p className="font-semibold text-sm text-[#413d3d]">
                    {language === 'es'
              ...
              {/* HSA/FSA Badge */}
              <div className="mb-4 bg-[#413d3d] text-white px-3 py-1.5 rounded-full text-xs font-medium flex items-center gap-1.5 w-fit">
              ...
              {/* Plan Selection */}
              <h3 className="text-base font-semibold mb-3">{t.choosePlan}</h3>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-4 rounded-2xl border-2 transition-all relative ${
                        isSelected ? 'border-[#cab172] bg-[#f5ecd8]' : 'border-gray-200 hover:border-gray-300'
                      }`}
                    >
                      <div className="flex items-center gap-3 w-full">
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${isSelected ? 'border-[#cab172] bg-[#cab172]' : 'border-gray-300'}`}>
                          {isSelected && (
                        ...
                        <div className="flex-1 text-left">
                          <span className="font-semibold text-[15px] text-[#413d3d]">{plan.type}</span>
                          {isSelected && plan.billing !== 'once' && (
                            <p className="text-xs text-[#413d3d]/60 mt-1">
                              {plan.billing === 'monthly'
                                ? (language === 'es' ? `$${plan.price}/mes` : `$${plan.price}/month`)
                                : plan.type.includes('3')
                                ? (language === 'es' ? `$${plan.price} cada 3 meses` : `$${plan.price} every 3 months`)
                                : (language === 'es' ? `$${plan.price} cada 6 meses` : `$${plan.price} every 6 months`)}
                            </p>
                          )}
                        </div>
                        <div className="text-right flex-shrink-0">
                          {originalMonthlyPrice && originalMonthlyPrice !== monthlyPrice && (
                            <span className="text-gray-400 line-through text-xs mr-1.5">${originalMonthlyPrice}/mo</span>
                          )}
                          <span className="font-bold text-[15px] text-[#413d3d]">${monthlyPrice}/mo</span>
                        </div>
                      </div>
              ...
              {/* Add-ons */}
              <h3 className="text-base font-semibold mb-3">{t.optionalAddons}</h3>
              <div className="space-y-3">
                {addons.map((addon) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3 w-full">
                        ...
                        />
                        <div className="flex-1 text-left">
                          <span className="font-medium text-sm text-[#413d3d]">{addon.name}</span>
                          <p className="text-xs text-[#413d3d]/60">{addon.description}</p>
                        </div>
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <span className="text-sm font-medium text-[#413d3d]">${addon.basePrice || addon.price}/{language === 'es' ? 'mes' : 'mo'}</span>
                          <div className={`w-5 h-5 rounded flex items-center justify-center flex-shrink-0 ${isSelected ? 'bg-white' : 'bg-transparent'}`}
          ...
          {currentStep === 3 && (
            <div className={`transition-all duration-300 ease-out ${isTransitioning ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
              <div className="space-y-2 mb-8">
                ...
                <p className="text-[15.5px] text-[#413d3d]/70 font-[350] leading-relaxed">{t.shippingSubtitle}</p>
              </div>

              {/* Contact Info */}
              {isDirectCheckout ? (
                <div className="mb-6">
                  <h3 className="text-base font-semibold text-[#413d3d] mb-4">{t.contactInfo}</h3>
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-[#413d3d] mb-1.5">
                        ...
                        <input
                          ...
                          className="w-full p-4 text-[17px] font-[530] bg-white border-2 border-black/10 rounded-2xl text-[#333] outline-none transition-all focus:border-[#cab172]"
                ...
                <div className="mb-6 p-4 rounded-2xl bg-gray-50">
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#413d3d] flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="flex-1">
                      ...
                      {patientData.email && <p className="text-sm text-[#413d3d]/70 mt-0.5">{patientData.email}</p>}
                      {patientData.phone && <p className="text-sm text-[#413d3d]/70">{patientData.phone}</p>}
                    </div>
                  </div>
                </div>
              ) : null}
              ...
              {/* Shipping Address */}
              <div className="mb-6">
                <h3 className="text-base font-semibold mb-4">{t.shippingAddress}</h3>

                {showAddressConfirmation && shippingAddress.addressLine1 && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                    <p className="text-sm font-medium text-blue-900 mb-2">
                      {language === 'es' ? '¿Es esta tu dirección de envío?' : 'Is this your shipping address?'}
                    </p>
                    ...
                    <div className="flex gap-3 mt-3">
                      <button onClick={() => setShowAddressConfirmation(false)} className="px-4 py-2 bg-[#413d3d] text-white rounded-full text-sm font-medium">
                        {language === 'es' ? 'Sí, usar esta' : 'Yes, use this'}
                      </button>
                      <button onClick={() => { setShowAddressConfirmation(false); setShippingAddress({ addressLine1: '', addressLine2: '', city: '', state: '', zipCode: '', country: 'US' }); }}
                ...
                <AddressAutocomplete value={shippingAddress} onChange={setShippingAddress} language={language} />
              </div>

              {/* Delivery Method */}
              <div className="mb-6">
                <h3 className="text-base font-semibold mb-4">{language === 'es' ? 'Método de Envío' : 'Delivery Method'}</h3>
                <div className="space-y-3">
                  <label className="block cursor-pointer">
                    <div className={`w-full text-left p-4 rounded-2xl border-2 transition-all ${!expeditedShipping ? 'border-[#cab172] bg-[#f5ecd8]' : 'border-gray-200'}`}>
                      <div className="flex items-center justify-between w-full">
                        <div className="flex items-center gap-3">
                          <input type="radio" name="shipping" checked={!expeditedShipping} onChange={() => setExpeditedShipping(false)} className="h-5 w-5 accent-[#cab172]" />
                          <div>
                            <p className="font-medium text-sm">{language === 'es' ? 'Estándar (5-7 días hábiles)' : 'Standard (5-7 business days)'}</p>
                            <p className="text-xs text-[#413d3d]/60">{language === 'es' ? 'Envío gratuito' : 'Free shipping'}</p>
                          </div>
                        </div>
                        <span className="font-medium text-sm">{language === 'es' ? 'GRATIS' : 'FREE'}</span>
                      </div>
                    </div>
                  </label>
                  <label className="block cursor-pointer">
                    <div className={`w-full text-left p-4 rounded-2xl border-2 transition-all ${expeditedShipping ? 'border-[#cab172] bg-[#f5ecd8]' : 'border-gray-200'}`}>
                      <div className="flex items-center justify-between w-full">
                        <div className="flex items-center gap-3">
                          <input type="radio" name="shipping" checked={expeditedShipping} onChange={() => setExpeditedShipping(true)} className="h-5 w-5 accent-[#cab172]" />
                          <div>
                            <p className="font-medium text-sm">{language === 'es' ? 'Rápido (3-5 días hábiles)' : 'Expedited (3-5 business days)'}</p>
                            <p className="text-xs text-[#413d3d]/60">{language === 'es' ? 'Recíbelo más rápido' : 'Get it faster'}</p>
                          </div>
                        </div>
          ...
          {currentStep === 4 && (
            <div className={`transition-all duration-300 ease-out ${isTransitioning ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
              <div className="space-y-2 mb-8">
                ...
                <p className="text-[15.5px] text-[#413d3d]/70 font-[350] leading-relaxed">
              ...
              {selectedMed && selectedPlanData && (
                <div className="mb-6 p-4 rounded-2xl bg-[#f5ecd8]">
                  <h3 className="text-sm font-semibold text-[#413d3d] mb-3">{t.orderSummary}</h3>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-[#413d3d]/70">{selectedMed.name} &middot; {selectedPlanData.type}</span>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className={expeditedShipping ? '' : 'text-green-600 font-medium'}>{expeditedShipping ? `$${shippingCost}` : 'FREE'}</span>
                    </div>
                    {discount > 0 && (
                      <div className="flex justify-between">
                        <span className="text-[#413d3d]/70">{language === 'es' ? 'Descuento' : 'Discount'}</span>
              ...
              <div className="mb-6 p-4 rounded-2xl bg-gray-50">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-[#413d3d]">{language === 'es' ? 'Envío' : 'Shipping'}</p>
                    <p className="text-sm text-[#413d3d]/80 mt-1">
                      {shippingAddress.addressLine1}{shippingAddress.addressLine2 ? `, ${shippingAddress.addressLine2}` : ''}
                      <br />{shippingAddress.city}, {shippingAddress.state} {shippingAddress.zipCode}
                    </p>
                  </div>
                  <button type="button" onClick={() => setCurrentStep(3)} className="text-sm text-[#cab172] hover:underline font-medium">
              ...
              {!promoApplied ? (
                <div className="mb-6 p-4 rounded-2xl bg-gray-50">
                  <p className="text-sm font-semibold text-[#413d3d] mb-3">
                  ...
                  <div className="flex gap-2">
                    <input
                      type="text"
                      value={promoCode}
                      onChange={(e) => { setPromoCode(e.target.value.toUpperCase()); if (promoError) setPromoError(null); }}
                      onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); applyPromo(); } }}
                      placeholder={language === 'es' ? 'Código' : 'Code'}
                      className="flex-1 p-3 text-sm font-[530] bg-white border-2 border-black/10 rounded-2xl text-[#333] outline-none transition-all focus:border-[#cab172] uppercase"
                      autoComplete="off"
                    />
                    <button type="button" onClick={applyPromo} className="px-4 py-2 rounded-full bg-[#413d3d] text-white text-sm font-medium">
                ...
                <div className="mb-6 flex items-center justify-between gap-2 bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <button type="button" onClick={removePromo} className="text-sm font-medium text-green-800 underline">
              ...
              {/* Payment Form */}
              <div className="mb-6">
                <h3 className="text-base font-semibold mb-4">{t.payment}</h3>
                <StripeProvider
                  amount={total}
                  ...
                  orderData={{
                    ...
                    addons: selectedAddons.map(id => { const addon = addons.find(a => a.id === id); return (addon as any)?.nameEn || addon?.name || ''; }).filter(Boolean),
                    expeditedShipping,
                    subtotal,
                    shippingCost,
                    total
              ...
              <div className="rounded-2xl p-4 bg-gray-50 mb-6">
                <p className="text-xs text-[#413d3d]/70 leading-relaxed">
      ...
      {/* Sticky Bottom CTA */}
      {currentStep < 4 && (
        <div className="sticky bottom-0 left-0 right-0 px-6 lg:px-8 py-4 pb-6" style={{ background: 'linear-gradient(to top, #ffffff 60%, transparent 100%)' }}>
          <div className="max-w-[480px] lg:max-w-[560px] mx-auto w-full">
            <button
              ...
              className="w-full flex items-center justify-center gap-3 py-4 px-8 rounded-full text-lg font-medium text-white transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              style={{ backgroundColor: '#413d3d' }}
            >
              <span>
                {currentStep === 1 ? t.continuePlan : currentStep === 2 ? t.continueShipping : t.continuePayment}
              </span>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
              </svg>
            </button>
            <p className="text-center mt-4" style={{ fontSize: '12px', color: 'rgba(65, 61, 61, 0.5)', lineHeight: '1.2' }}>
              &copy; 2026 EONPro, LLC. All rights reserved.<br />Exclusive and protected process.
            </p>
          </div>
        </div>
      )}
      ...
      {currentStep === 4 && (
        <div className="px-6 pb-6">
          <p className="text-center" style={{ fontSize: '12px', color: 'rgba(65, 61, 61, 0.5)', lineHeight: '1.2' }}>
            &copy; 2026 EONPro, LLC. All rights reserved.<br />Exclusive and protected process.
          </p>
        </div>
      )}
