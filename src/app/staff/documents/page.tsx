...
import { FileText, Search, Upload, Download, Eye, Trash2, FolderOpen, Filter, Clock } from "lucide-react";

interface Document {
  id: string;
  name: string;
  patientName: string;
  category: "medical-records" | "lab-results" | "insurance" | "consent-forms" | "prescriptions" | "other";
  uploadedBy: string;
  uploadedAt: string;
  size: string;
...
export default function StaffDocumentsPage() {
  ...
  const getCategoryColor = (category: string) => {
    switch(category) {
      ...
      case "consent-forms": return "bg-yellow-100 text-yellow-800";
      case "prescriptions": return "bg-pink-100 text-pink-800";
      ...
      {selectedDocuments.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button className="px-3 py-1 text-sm bg-white text-gray-700 rounded hover:bg-gray-50">
              Download Selected
            </button>
              ...
              <tbody>
                {filteredDocuments.map((doc) => (
                  <tr key={doc.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">{doc.patientName}</td>
                    <td className="py-3 px-4">
