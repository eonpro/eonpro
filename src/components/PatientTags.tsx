    ...
    <div className="flex flex-wrap gap-2 mb-6">
      {tags.map((tag: string) => {
        ...
        return (
          <span
            key={tag}
            className={`group relative px-4 py-2 pr-8 rounded-full text-sm font-medium border ${tagStyle} ${isRemoving ? 'opacity-50' : ''}`}
          >
            #{tag}
            <button
              onClick={() => removeTag(tag)}
              disabled={isRemoving}
              className="absolute right-2 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full flex items-center justify-center hover:bg-black/10 transition-colors"
              title="Remove tag"
            >
              {isRemoving ? (
