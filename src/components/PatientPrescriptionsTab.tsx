...
export default function PatientPrescriptionsTab({
  ...
  const recentOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      const isErrorOrDeclined =
        order.status === 'error' || order.status === 'cancelled' || order.status === 'declined';
      ...
      <section className="rounded-xl border bg-white p-3 shadow sm:p-4 md:p-6">
        <div className="mb-3 flex flex-col gap-1 sm:mb-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span className="text-xs text-gray-500 sm:text-sm">
            {orders.length} prescription{orders.length !== 1 ? 's' : ''}
          </span>
        </div>
              ...
              {orders.map((order: any) => (
                ...
                >
                  ...
                  <div className="mb-2">
                    {order.rxs?.map((rx: any) => (
                      <p key={rx.id} className="text-sm font-medium text-gray-900">
                        ...
                        {rx.strength && (
                          <span className="font-normal text-gray-500"> ({rx.strength})</span>
                        )}
                        {rx.quantity && (
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500">
                    {order.provider && <span>Dr. {order.provider.lastName}</span>}
                    {order.shippingMethod && (
                      <span>
                        {shippingLabelMap.get(order.shippingMethod) ?? order.shippingMethod}
                      </span>
                    )}
                    {order.lifefileOrderId && <span>Order #{order.lifefileOrderId}</span>}
                  </div>
            ...
            <div className="w-full max-w-md rounded-xl bg-white shadow-xl">
              <div className="p-6">
                ...
                <p className="mb-3 text-sm text-gray-700">
                  This patient already has {recentOrders.length} prescription
                  {recentOrders.length > 1 ? 's' : ''} in the last {DUPLICATE_WINDOW_DAYS} days:
                </p>
                  ...
                  {recentOrders.slice(0, 3).map((order) => (
                    <li key={order.id} className="flex items-center gap-2 text-sm text-red-800">
                      ...
                      <span className="font-medium">
                        {order.rxs.map((rx) => rx.medName || rx.medicationKey).join(', ') ||
                          'Unknown'}
                      </span>
