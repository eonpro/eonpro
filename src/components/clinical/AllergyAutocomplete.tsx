...
const CATEGORY_LABELS: Record<string, { label: string; color: string }> = {
  drug: { label: 'Drug', color: 'bg-red-50 text-red-600' },
  food: { label: 'Food', color: 'bg-amber-50 text-amber-600' },
  environmental: { label: 'Env', color: 'bg-blue-50 text-blue-600' },
};
...
export default function AllergyAutocomplete({ value, onChange, placeholder, className }: Props) {
  ...
  const selectResult = useCallback((item: AllergyResult) => {
  ...
  return (
    <div className="relative">
      <textarea
        ...
        placeholder={placeholder ?? 'Type to search allergies...'}
        rows={2}
      ...
      {isOpen && results.length > 0 && (
        ...
        >
          {results.map((item, i) => {
            const cat = CATEGORY_LABELS[item.category] ?? CATEGORY_LABELS.drug;
            return (
              <button
                key={`${item.name}-${i}`}
                type="button"
