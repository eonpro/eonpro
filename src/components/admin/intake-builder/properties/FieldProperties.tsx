...
import type { FormField, FormStep, FieldOption } from '../state/builderTypes';
...
export default function FieldProperties({
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 border-b border-gray-200 bg-gray-50/50 px-3 py-2">
        <Type className="h-4 w-4 text-indigo-500" />
        <div className="min-w-0 flex-1">
          <span className="block truncate text-xs font-medium text-gray-500">{typeLabel}</span>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {activeTab === 'validation' && (
          <FieldValidationEditor step={step} field={field} onUpdateField={onUpdateField} />
        )}
        {activeTab === 'logic' && (
          <FieldLogicEditor step={step} field={field} steps={steps} onUpdateField={onUpdateField} />
        )}
      </div>
    </div>
  );
