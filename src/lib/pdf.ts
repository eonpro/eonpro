...
export async function generatePrescriptionPDF(data: PrescriptionPdfData) {
  ...
  for (let rxIndex = 0; rxIndex < prescriptions.length; rxIndex++) {
    ...
    if (currentPageNumber === 1 && rxIndex === 0) {
      ...
      drawText(currentPage, "ELECTRONIC PRESCRIPTION ORDER", margin, cursor, { bold: true, size: 11 });
      cursor -= 18;
      ...
      drawRow(currentPage, [
        { label: "ADDRESS", value: data.patient.address1 + (data.patient.address2 ? `, ${data.patient.address2}` : ""), width: 400 },
      ], margin, cursor);
      cursor -= 18;
      ...
      cursor += 50;
      drawText(currentPage, "ELECTRONIC PRESCRIPTION ORDER - CONTINUED", margin, cursor, { bold: true, size: 12 });
      cursor -= 18;
      drawText(currentPage, `Patient: ${data.patient.firstName} ${data.patient.lastName} (DOB: ${data.patient.dob})`, margin, cursor, { size: 10 });
      cursor -= 25;
      drawLine(currentPage, cursor);
      cursor -= 25;
      isNewPage = false;
  ...
  // Footer Information
  drawText(currentPage, `Submitted electronically by prescriber: ${data.provider.name.toUpperCase()}`, margin, cursor, { size: 10 });
  cursor -= 14;
