...
const DATE_AND_TIME =
  /(\d{1,2})\/(\d{1,2})\/(\d{4})\s*\/\s*(\d{1,2}):(\d{2})\s*(EST|EDT|CST|CDT|PST|PDT|UTC)?/i;
...
function parseValueAndFlag(raw: string): {
  value: string;
  valueNumeric: number | null;
  flag: string | null;
} {
...
function getCategory(testName: string): string {
  ...
  if (
    /GLUCOSE|BUN|CREATININE|EGFR|SODIUM|POTASSIUM|ALBUMIN|BILIRUBIN|AST|ALT|ALKALINE|CARBON|CALCIUM|PROTEIN|GLOBULIN|CHLORIDE/i.test(
      key
    )
  )
    return 'metabolic';
  ...
  if (
    /WBC|RBC|HEMOGLOBIN|HEMATOCRIT|PLATELET|MCV|MCH|NEUTROPHIL|LYMPHOCYTE|MONOCYTE|EOSINOPHIL|BASOPHIL/i.test(
      key
    )
  )
    return 'blood';
...
function looksLikeReference(s: string): boolean {
  ...
  if (
    /^[\d.]+/.test(t) &&
    (/\s*(?:mg\/dL|mmol\/L|U\/L|pg\/mL|ng\/mL|g\/dL|%|Thousand\/uL|Million\/uL)\s*(?:\(calc\))?\s*$/i.test(
      t
    ) ||
      /\(calc\)\s*$/i.test(t))
  )
...
export function parseQuestText(fullText: string): QuestParsedResult {
  ...
  const notNameLike =
    /^\s*(Specimen|Collected|Reported|Test\s+Name|Reference|Page\s+\d|Quest|Report\s+Status|SEE\s+NOTE|\d)/i;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const isKnownTest =
      testNameFromParts.length > 0 &&
      (QUEST_TEST_NAMES.has(firstNorm) || isKnownTestName(parts[0]));
    ...
    if (isKnownTest && restAfterName.length >= 1) {
      ...
      for (let k = 0; k < restAfterName.length; k++) {
        if (isValueToken(restAfterName[k])) {
          ...
          if (k + 1 < restAfterName.length && looksLikeReference(restAfterName[k + 1]))
            refToken = restAfterName[k + 1];
          break;
        }
      }
      if (
        !valueToken &&
        restAfterName.length >= 1 &&
        isValueToken(restAfterName[restAfterName.length - 1])
      )
        valueToken = restAfterName[restAfterName.length - 1];
      if (
        !valueToken &&
        restAfterName.length >= 2 &&
        isValueToken(restAfterName[restAfterName.length - 2])
      )
        valueToken = restAfterName[restAfterName.length - 2];
      if (
        !refToken &&
        restAfterName.length >= 1 &&
        looksLikeReference(restAfterName[restAfterName.length - 1])
      )
        refToken = restAfterName[restAfterName.length - 1];
      if (
        !refToken &&
        restAfterName.length >= 2 &&
        looksLikeReference(restAfterName[restAfterName.length - 2])
      )
        refToken = restAfterName[restAfterName.length - 2];
    ...
    if (parts.length >= 3 && isValueToken(last)) {
      ...
      const testName = parts
        .slice(0, looksLikeReference(secondLast) ? -2 : -1)
        .join(' ')
        .trim();
...
/** Maximum PDF size for parsing (bytes). Align with upload route limit. */
