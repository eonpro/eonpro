...
const PHI_PATTERNS = {
  ...
  // Street addresses
  STREET_ADDRESS:
    /\b\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Lane|Ln|Drive|Dr|Court|Ct|Circle|Cir|Plaza|Pl|Terrace|Ter|Way|Parkway|Pkwy)\b/gi,
...
export function anonymizeText(text: string): string {
  ...
  anonymized = anonymized.replace(PHI_PATTERNS.DATES, (match) => {
    ...
    if (
      match.toLowerCase().includes('today') ||
 ...
 */
export function createAnonymizationMap(originalData: Record<string, any>): Map<string, string> {
 ...
 */
export function logAnonymization(userId: number, purpose: string, dataType: string): void {
