...
export async function generateIntakePdf(intake: NormalizedIntake, patient: Patient) {
  ...
  cursor -= 10;
  drawText(page, font, `Submission ID: ${intake.submissionId}`, margin, cursor, 10, LIGHT_TEXT);
  cursor -= 30;
  ...
  displaySections.forEach((section: any) => {
    ...
    section.entries.forEach((entry: { label?: string; value?: string }, index: number) => {
      ...
      const valueY =
        cursor - 18 - (labelLines && labelLines.length > 1 ? (labelLines.length - 1) * 14 : 0);
      renderWrappedText(
        page,
        font,
        formattedValue,
...
function drawCard(
  ...
  entries.forEach((entry, index) => {
    ...
    const formattedValue =
      entry.label === 'Address' ? entry.value : formatAnswerValue(entry.value || '—');
    renderWrappedText(
      page,
      font,
      formattedValue,
...
function buildDisplaySections(intake: NormalizedIntake): PdfSection[] {
    ...
    const entries = sectionConfig.fields
      .map((field: any) => {
        ...
        if (directMatch && directMatch.value) {
          ...
          return {
            label: field.label ?? directMatch.label,
            value: formatAnswerValue(directMatch.value),
          };
        }
        ...
        if (labelKey) {
          ...
          if (labelMatch) {
            ...
            return {
              label: field.label ?? labelMatch.label,
              value: formatAnswerValue(labelMatch.value),
            };
          }
        }
        return null;
  ...
  intake.answers
    .filter((entry: any) => {
      ...
      if (
        excludeFromAdditional.some((exclude: any) => labelLower.includes(exclude.toLowerCase()))
      ) {
        return false;
      }
...
function drawStamp(page: any, font: any, x: number, cursor: number, intake: NormalizedIntake) {
  ...
  drawText(page, font, `Submission ID: ${intake.submissionId}`, x + 12, cursor - 58, 8, LIGHT_TEXT);
}
