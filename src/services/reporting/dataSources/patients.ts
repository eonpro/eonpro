...
const definition: DataSourceDef = {
  ...
  filters: [
    ...
    },
    {
      ...
      options: [
        ...
        { value: 'PENDING_COMPLETION', label: 'Pending' },
  ...
  groupByOptions: [
    ...
    { id: 'profileStatus', label: 'By Status' },
...
async function execute(config: ReportConfig): Promise<ReportResult> {
  ...
  const patients = await prisma.patient.findMany({
    ...
    select: {
      ...
      salesRepAssignments: {
        where: { isActive: true },
        select: { salesRep: { select: { firstName: true, lastName: true } } },
        take: 1,
      },
  ...
  const summary = {
    ...
    conversionRate:
      patients.length > 0
        ? Math.round(
            (patients.filter((p) => p.payments.length > 0).length / patients.length) * 10000
          ) / 100
        : 0,
  };
