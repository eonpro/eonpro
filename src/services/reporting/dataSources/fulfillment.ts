...
async function execute(config: ReportConfig): Promise<ReportResult> {
  ...
  const rows: ReportRow[] = shipments.map((s) => {
    const daysInTransit =
      s.shippedAt && s.actualDelivery
        ? Math.round((s.actualDelivery.getTime() - s.shippedAt.getTime()) / 86_400_000)
        : s.shippedAt
          ? Math.round((Date.now() - s.shippedAt.getTime()) / 86_400_000)
          : null;
    return {
      id: s.id,
      date: s.createdAt.toISOString(),
      patientId: s.patientId,
      clinicName: (s as any).clinic?.name || '',
  ...
  const summary = {
    ...
    deliveryRate:
      shipments.length > 0 ? Math.round((delivered / shipments.length) * 10000) / 100 : 0,
  };
