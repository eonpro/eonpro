...
export default function TRTLandingStep({
  basePath,
  nextStep,
}: TRTLandingStepProps) {
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 flex flex-col px-6 lg:px-8 pt-8 lg:pt-12 pb-6 max-w-md lg:max-w-2xl mx-auto w-full">
        ...
        <div className="text-left mb-6">
          ...
          >
            {isSpanish ? (
              <>Recupera tu energía,<br />impulso y vitalidad.</>
            ) : (
              <>Restore your energy,<br />drive and vitality.</>
            )}
          </h1>
          <p className={`page-subtitle leading-tight mt-3 transform transition-all duration-700 ease-out delay-300 ${animate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            {isSpanish ? (
              <>Descubre si la Terapia de Reemplazo de<br />Testosterona es adecuada para ti.</>
            ) : (
              <>See if Testosterone Replacement Therapy<br />is right for you.</>
            )}
          </p>
        </div>
