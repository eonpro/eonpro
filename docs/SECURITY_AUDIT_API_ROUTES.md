...
**Issue**: Returned patient counts, provider counts, billing plans for ALL clinics without authentication.
...
**Fix Applied**: Added `verifyAuth`, requires admin/super_admin role, non-super-admin can only update own clinic.
...
**Issue**: Exposed clinic configuration including Stripe account presence, Lifefile credentials presence, patient/provider counts - all without auth.
