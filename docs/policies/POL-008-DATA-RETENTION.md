...
This policy establishes requirements for retaining and securely disposing of data processed by EONPRO Telehealth Platform, ensuring compliance with HIPAA, state regulations, and business requirements while minimizing data exposure risk.
...
function isEligibleForDisposal(patient: Patient): boolean {
  const retentionYears = patient.isMinor 
    ? Math.max(6, 21 - patient.birthYear + 6)
    : 6;
