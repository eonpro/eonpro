...
Pull data directly from Airtable using our sync API. This is more reliable and doesn't require
...
```json
{
  ...
  "result": {
    ...
    "errors": [{ "recordId": "rec789", "error": "Missing email" }]
  }
}
```
...
The webhook automatically detects the treatment type as `weight_loss` if any of these fields are
present:
