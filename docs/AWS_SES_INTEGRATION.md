...
4. **Set Up DKIM**
   ...
   - Add CNAME records to your DNS (see [DNS on Vercel](#dns-for-ses-when-your-domain-is-on-vercel) below)
...
1. **Open Domains in Vercel**
   ...
   - Or: **Project** → **Settings** → **Domains** and use **Edit** on the root domain (e.g. `eonpro.io`).
...
2. **Find where to add DNS records**
   ...
   - If you don’t see it, the domain may be using external nameservers; add the records at the provider that hosts DNS for `eonpro.io`.
...
3. **Add the 3 DKIM CNAME records** (from SES “Domain DNS records” modal):
   ...
   - In Vercel, **Name** is usually the part *before* `eonpro.io` (e.g. `ty2xrmci77xerey7rnl6rfcecfdhz55e._domainkey`). If it asks for a full name, use e.g. `ty2xrmci77xerey7rnl6rfcecfdhz55e._domainkey.eonpro.io`.
...
5. **Save and wait for verification**
   ...
   - In **AWS SES** → **Verified identities** → **eonpro.io**, DKIM status will change to **Verified** once the CNAMEs are visible.

6. **Custom MAIL FROM (fix “MAIL FROM record is not aligned”)**
   - The SES modal showed “No MAIL FROM records found.” Aligning the envelope sender (Return-Path) with your domain improves deliverability and [DMARC compliance](https://docs.aws.amazon.com/ses/latest/dg/send-email-authentication-dmarc.html). **In SES:** Configuration → **Identities** → **eonpro.io** → **Authentication** → **Custom MAIL FROM domain** → **Edit**. Enable **Use a custom MAIL FROM domain**, set **MAIL FROM domain** to a subdomain of `eonpro.io` not used for sending/receiving (e.g. `bounce.eonpro.io`). **Behavior on MX failure:** **Use default MAIL FROM domain**. Save. **In Vercel DNS:** add the MX and TXT (SPF) records from SES **Publish DNS records** (MX: name `bounce`, value `10 feedback-smtp.us-east-2.amazonses.com`; TXT: name `bounce`, value `"v=spf1 include:amazonses.com ~all"`). Exactly one MX on the MAIL FROM subdomain. Verification can take minutes to 72 hours.
...
This wires SES send/delivery/bounce/complaint events to your app so you can track and react (e.g. your existing `/api/webhooks/ses-bounce` handler).
...
5. **Event types:** enable **Send**, **Delivery**, **Bounce**, **Complaint** (and **Reject** if you want).
6. **Add destination**.
...
6. AWS will send a **SubscriptionConfirmation** to that URL; your app’s webhook already handles it and confirms automatically. In SNS, the subscription status should change to **Confirmed** after the first request.

**5. Use the Configuration Set when sending**

Set the configuration set name in your environment so all SES sends use it:

- In **production:** set `AWS_SES_CONFIGURATION_SET=eonpro-email-events` in your deployment env (e.g. Vercel → Project → Settings → Environment Variables).
- In **local:** add the same to `.env.local` or `.env.production.local` if you test against production SES.

The app only attaches the configuration set to outbound emails when `AWS_SES_CONFIGURATION_SET` is set; if it’s empty, sends are unchanged.

**6. Verify**

- Send a test email (e.g. from your app or the SES console).
- In **SNS** → **Topics** → **ses-eonpro-events** → **Subscriptions**, the subscription should be **Confirmed**.
- Check your app logs or `email_log` (if you log there) for webhook requests; you should see SNS notifications for Send/Delivery (and Bounce/Complaint when they occur).

**Optional: CloudWatch**

- In the Configuration Set’s event destination you can also add a **CloudWatch** destination to get metrics (e.g. send count, bounce count) in CloudWatch without going through your webhook.
...
- [ ] Set up monitoring (Configuration Set + SNS → webhook); see [Set up SES monitoring](#set-up-ses-monitoring-configuration-set--sns)
